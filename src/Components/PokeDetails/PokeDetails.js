...
const PokeDetail = () => {
    ...
    useEffect(() => {
        if(state.pokemons.length > 0){
            fetchPokeData(state.pokemons[state.detailPokemonId].url)
            .catch(e => console.log(e))
            .then(data => { dispatch({type : 'SHOW_DETAIL', payload :data })         
                  })
        }

        }, [state.detailPokemonId])
    return (
          <React.Fragment>
          <article className="detailBox boxElem">
          { state.displayError &&   <div className="error">
                VOUS AVEZ TROP DE POKEMON 
            </div>}
            {state.pokemonToDetails != null &&  (
              <span>
                ...
                <img className="pkmnPict" src={state.pokemonToDetails.sprites.front_default}/>
                <ul>
                {/* "types": [
    {
      "slot": 1,
      "type": {
        ...
        "url": "https://pokeapi.co/api/v2/type/12/"
                    ...
                    <p>Type :</p>
                      <ul>
                      {state.pokemonToDetails.types.map((elem,i) => {
                          return <li key={i}>{elem.type.name} </li>
                        })}
                      </ul>
                    </li>
                    {/* "stats": [
      ...
      "stat": {
        ...
        "url": "https://pokeapi.co/api/v2/stat/1/"
                    ...
                    <li> Stats: 
                      <ul>
                        {state.pokemonToDetails.stats.map((elem,i)=> {
                          return <li key={i}>{elem.stat.name} : {elem.base_stat}</li>
                        })}
                      </ul>
                    </li>
                </ul>
...
/* tête du JSON recuperer en details 
{
{
  "abilities": [
    {
